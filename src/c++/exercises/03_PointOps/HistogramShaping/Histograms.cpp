...
namespace ip
{
	...
	*/
	...
	void histogramShapeGaussian(const Mat& image, Mat& processed, double mean, double stdDev) {
		const int NUMBER_BINS = 256;
		...
		double dstHist[NUMBER_BINS];
		double sum = 0.0;

		for (int i = 0; i < NUMBER_BINS; i++) {
			dstHist[i] = exp(-0.5 * (i - mean) * (i - mean) / (stdDev * stdDev));
			sum += dstHist[i];
		}
		for (int i = 0; i < NUMBER_BINS; i++) {
			dstHist[i] *= (image.rows * image.cols) / sum;
		}

		// Calculate cumulative Gaussian target histogram
		unsigned dstCumHist[NUMBER_BINS];
		sum = 0.0;
		for (int i = 0; i < NUMBER_BINS; i++) {
			sum += dstHist[i];
			dstCumHist[i] = (unsigned)(sum + 0.5);
		}
		...
		for (int gSource = 0; gSource < 256; gSource++) {
			for (int gTarget = 0; gTarget < NUMBER_BINS; gTarget++) {
				if ((cumHist[gSource] <= dstCumHist[gTarget]) || (gTarget == (NUMBER_BINS - 1))) {
