...
/* Defines */
#define CAMERA_ID 0
#define WIN_NAME_CAMERA "Dynamic template matching"
#define WIN_NAME_TEMPLATE "Template"
#define TRACKBAR_SIZE "Size"
#define TRACKBAR_UPDATE_CORR "Update NCF"
#define ROI_SIZE 50
#define MIN_UPDATE_CORR 70
...
void onTrackbarBoxUpdateCorr(int value, void* pointer);
...
double minUpdateCorr = MIN_UPDATE_CORR / 100.0;
...
int main()
{
	...
	Rect2i roi((frame.cols - ROI_SIZE) / 2, (frame.rows - ROI_SIZE) / 2, roiSize, ROI_SIZE);
	...
	createTrackbar(TRACKBAR_UPDATE_CORR, WIN_NAME_CAMERA, NULL, 100, onTrackbarBoxUpdateCorr, NULL);
	setTrackbarMin(TRACKBAR_SIZE, WIN_NAME_CAMERA, 1);
	setTrackbarPos(TRACKBAR_SIZE, WIN_NAME_CAMERA, ROI_SIZE);
	setTrackbarPos(TRACKBAR_UPDATE_CORR, WIN_NAME_CAMERA, MIN_UPDATE_CORR);
	...
	while (camera.getNextFrame(frame, &grayImage)) {
		...
		// Update template image if correlation >= min. update value
		if (bestMatchCorr >= minUpdateCorr) {
			Mat templateImage = grayImage(roi).clone();
...
/*! Trackbar callback to change min. correlation value to update template image.
*
* \param value Slider value (correlation value in [0, 100] corresponding to [0.0, 1.0])
* \param pointer Not used
*/
void onTrackbarBoxUpdateCorr(int value, void* pointer) {
	minUpdateCorr = value / 100.0;
}
